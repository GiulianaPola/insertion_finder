...
def warn_with_traceback(msg, cat, fname, lno, file=None, line=None):
    ...
    """
    log = sys.stderr if file is None else file
    traceback.print_stack(file=log)
    log.write(warnings.formatwarning(msg, cat, fname, lno, line))
    log.flush()
...
HELP_MSG = HELP_MSG + 'Usage: insertion_finder.py -q <query file> -d <database file> -run local\n'
HELP_MSG = HELP_MSG + '\tinsertion_finder.py -q <query file> -d <database file> -run local -tab <BLASTn table file>\n'
HELP_MSG = HELP_MSG + '\tinsertion_finder.py -q <query file> -run web -tab <BLASTn table file>\n'
HELP_MSG = HELP_MSG + '\tinsertion_finder.py -q <query file> -run web\n'
HELP_MSG = HELP_MSG + '\nMandatory parameters:\n'
HELP_MSG = HELP_MSG + '-q <fasta or multifasta file>\tSequence to search with\n'
HELP_MSG = HELP_MSG + '-d <multifasta file>\tDatabase to BLAST against (for -run local)\n'
HELP_MSG = HELP_MSG + '-run <local|web>\tchoice of running local or web BLAST search\n'
HELP_MSG = HELP_MSG + '\nOptional parameters:\n'
HELP_MSG = HELP_MSG + '-tab <table file>\tBLASTn search result table (fields: qseqid,sseqid,qcovs,qlen,slen,qstart,qend)\n'
HELP_MSG = HELP_MSG + '-org <int>\tTaxid(s) to restrict the database of the BLASTn search (for -run web)\n'
HELP_MSG = HELP_MSG + "-out <path>\tOutput directory (default: output_dir1)\n"
HELP_MSG = HELP_MSG + "-enddist <int>\tMaximum distance between block tip and query tip in base pairs(bp) (default: 50)\n"
HELP_MSG = HELP_MSG + "-minlen <int>\tMinimum element's length in base pairs(bp) (default: 4000)\n"
HELP_MSG = HELP_MSG + "-maxlen <int>\tMaximum element's length in base pairs(bp) (default: 50000)\n"
HELP_MSG = HELP_MSG + "-mincov <int>\tMinimum % query coverage per subject (default: 30)\n"
HELP_MSG = HELP_MSG + "-maxcov <int>\tMaximum % query coverage per subject (default: 90)\n"
HELP_MSG = HELP_MSG + "-cpu <int>\tNumber of threads to execute the local blastn search (default: 18)\n"
HELP_MSG = HELP_MSG + "-color <int>\tThe RGB color of the element that is shown by the feature table, three integers between 0 and 255 separated by commas (default: 255,0,0)"
...
def validate_args(args):
    ...
    if is_valid:
        try:
            out_path_base = args.out if args.out else 'output_dir'
    ...
    try:
        for record in SeqIO.parse(fasta_fpath, "fasta"):
            ...
            if record_len > MAX_BP_PER_BATCH:
                warn_msg = "WARN: Sequence '{}' ({} bp) exceeds MAX_BP_PER_BATCH ({}) and will be put in its own batch.\n".format(
                    record.id, record_len, MAX_BP_PER_BATCH
                )
                log.write(warn_msg)
            ...
            if current_batch_records and \
               (record_len > MAX_BP_PER_BATCH or current_batch_bp + record_len > MAX_BP_PER_BATCH):
                ...
                log_msg = "INFO: Writing batch {} ({} sequences, {} bp) to file '{}'\n".format(
                    batch_num, len(current_batch_records), current_batch_bp, out_fpath
                )
                log.write(log_msg)
        ...
        if current_batch_records:
            out_fpath = os.path.join(parts_dir, "query_part_{}.fasta".format(batch_num))
            log_msg = "INFO: Writing final batch {} ({} sequences, {} bp) to file '{}'\n".format(
                batch_num, len(current_batch_records), current_batch_bp, out_fpath
            )
            log.write(log_msg)
            SeqIO.write(current_batch_records, out_fpath, "fasta")
        ...
        summary_msg = "INFO: FASTA splitting complete. Created {} batch file(s) in '{}'.\n".format(
            len(out_files), parts_dir
        )
...
def run_blast_batch(batch_fpath, batch_idx, total_batches, blast_params, out_fpath=None):
    ...
    if out_fpath:
        ...
        blast_parts_dir = blast_params.get('current_blastn_parts_dir', os.path.join(blast_params["out"], "blastn_parts"))
        if not os.path.exists(blast_parts_dir):
            os.makedirs(blast_parts_dir)
        batch_out_fpath = os.path.join(blast_parts_dir, "blast_batch_{}.tab".format(batch_idx + 1))
    ...
    if blast_params.get('run') == 'web':
        blst_cmd = NcbiblastnCommandline(db='nt', remote=True, **blast_args)
        if 'org' in blast_params and blast_params['org']:
            blst_cmd.entrez_query = "'{}'".format(blast_params['org'])
    elif blast_params.get('run') == 'local':
        n_threads = max(1, int(blast_params.get('cpu', 1)))
        blst_cmd = NcbiblastnCommandline(db=blast_params.get('d'), num_threads=n_threads, **blast_args)
    else:
        err_msg = "ERROR: Invalid run type '{}' for {}.".format(blast_params.get('run'), log_suffix)
        sys.stderr.write(err_msg + "\n")
        if log: log.write("\n" + err_msg + "\n"); log.flush()
        if blast_log_fh: blast_log_fh.write(err_msg + "\n"); blast_log_fh.flush()
    ...
    attempt = 0
    cpu_warn = False
    FATAL_ERR_PATTERNS = ["Failed to connect", "Connection refused", "Service not found", "Name or service not known", "failure in name resolution", "invalid output format", "Unknown output format"]

    while attempt < max_attempts:
        cmd_string = str(blst_cmd)
        ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        if blast_log_fh:
            blast_log_fh.write("\n--- {} BLAST Attempt {}/{}, {} at {} ---\nCMD: {}\n".format(
                blast_params.get('run').upper(), attempt + 1, max_attempts, log_suffix, ts, cmd_string))
            blast_log_fh.flush()
        
        try:
            proc = subprocess.Popen(cmd_string, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, shell=True)
            stdout, stderr = proc.communicate()
            retcode = proc.returncode
            if retcode != 0:
                try:
                    raise subprocess.CalledProcessError(retcode, cmd_string, output=stdout, stderr=stderr)
                except TypeError:
                    raise subprocess.CalledProcessError(retcode, cmd_string, output=stdout)

            if stderr and blast_log_fh:
                blast_log_fh.write("STDERR:\n{}\n".format(stderr)); blast_log_fh.flush()

            if "consumed a large amount of server CPU time" in stderr:
                cpu_warn = True
                if blast_log_fh: blast_log_fh.write("NCBI CPU usage warning detected.\n"); blast_log_fh.flush()

            is_fatal = any(pattern in stderr for pattern in FATAL_ERR_PATTERNS)
            if not is_fatal:
                if blast_log_fh: blast_log_fh.write("SUCCESS: BLAST for {} complete.\n".format(log_suffix)); blast_log_fh.flush()
                return 'SUCCESS', batch_out_fpath, cpu_warn
            else:
                err_msg = stderr if stderr else ""
                sys.stderr.write(err_msg)
                raise subprocess.CalledProcessError(retcode, " ".join(cmd_string))
        except subprocess.CalledProcessError as e:
            err_detail = e.output if e.output else str(e)
            if "BLAST query/options error" in err_detail or "Argument" in err_detail or any(p in err_detail for p in ["unknown output format", "invalid output format"]):
                err_msg = "ERROR: [{}]: Permanent BLAST error detected. Not retrying. See blastn.log.\n".format(log_suffix)
                sys.stderr.write(err_msg)
                if log: log.write("\n" + err_msg); log.flush()
                if blast_log_fh: blast_log_fh.write(err_msg); blast_log_fh.flush()
                return 'FAILURE', None, cpu_warn
            
            if blast_log_fh:
                blast_log_fh.write("ERROR: BLAST for {} failed (attempt {}/{}), code {}.\nDetail: {}\n".format(log_suffix, attempt + 1, max_attempts, e.returncode, err_detail.strip()))
                blast_log_fh.flush()
            
            attempt += 1
            if attempt < max_attempts:
                wait_time = (base_backoff_delay * (2 ** (attempt - 1))) + random.uniform(0, 5)
                retry_msg = "BLAST for [{}] failed. Retrying in {:.1f}s (attempt {}/{})...".format(log_suffix, wait_time, attempt + 1, max_attempts)
                sys.stderr.write(retry_msg + "\n")
                if log: log.write("\n" + retry_msg + "\n")
                if blast_log_fh: blast_log_fh.write(retry_msg + "\n")
                log.flush(); blast_log_fh.flush()
                time.sleep(wait_time)
            else:
                final_fail_msg = "ERROR: [{}] BLAST failed after {} retries.".format(log_suffix, max_attempts)
                sys.stderr.write(final_fail_msg + "\n")
                if log: log.write("\n" + final_fail_msg + "\n")
                if blast_log_fh: blast_log_fh.write(final_fail_msg + "\n")
                return 'FAILURE', None, cpu_warn
...
def run_blast(params, query_fpath, out_fname="blastn.tab", t_start=None):
    ...
    try:
        ...
        if total_bp <= max_bp or num_seqs <= 1:
            ...
            if not batch_fpaths:
                if log: log.write("WARN: No FASTA batches were created from query.\n")
                return datetime.now() - t_start, None
            ...
            for i, batch_path in enumerate(batch_fpaths):
                ...
                if cpu_warn and params.get('run') == 'web':
                    ...
                    if log: log.write("WARN: CPU usage high. Delay increased to {}s.\n".format(params['web_inter_batch_delay']))
...
def write_missing_queries(missing_qids, orig_fasta, out_fasta):
    ...
    try:
        ...
        if count > 0:
            if log: log.write("Wrote {} missing queries to {}\n".format(count, out_fasta))
            return True
        else:
            if log: log.write("WARN: Could not find any records for {} missing IDs in {}\n".format(len(missing_qids), orig_fasta))
            return False
    ...
    for i, read in enumerate(reads):
        try:
            if len(read) == 2:
                ...
                if log: log.write("WARN: Skipping invalid alignment format [{}]: {}\n".format(i, read))
        except (ValueError, TypeError):
            if log: log.write("WARN: Skipping non-integer coordinate [{}]: {}\n".format(i, read))
...
def extract_seq(seq_content, qid, estart, eend):
    ...
    if header_pos == -1:
        if log: log.write("WARN: Header '{}' not found for extraction.\n".format(header))
        return ""
...
def write_element_files(qid, qseq_content, elem_start, elem_end, output_dir, color_rgb, log_fh):
    ...
    try:
        ...
        sys.stderr.write(err_msg)
        if log_fh: log_fh.write(err_msg + traceback.format_exc())
...
def rejection_reason(reasons_list, params):
    ...
    priority_order = {
        ...
        "REJECTED: Element length not in range ({minlen}-{maxlen}bp)".format(minlen=params['minlen'], maxlen=params['maxlen']): 4
    }
    ...
    try:
        sorted_details = sorted(list(set(all_details_for_reason)), key=lambda x: -float(x.split('(')[-1].split('%')[0].split('bp')[0]))
...
def main(params, qseq_content, log, blast_log_fh):
    ...
    if params.get('tab') is None:
        ...
        log.write('BLASTn finished in: {}\n'.format(init_blast_runtime)); log.flush()
    ...
    while rerun_count < MAX_COMPLEMENTARY_RUNS:
        ...
        if supp_params.get('run') != 'local': supp_params['run'] = 'web'
        log.write("Supplementary BLAST mode: '{}'.\n".format(supp_params['run'])); log.flush()
        
        tmp_supp_blast_tab = os.path.join(params["out"], "blastn_extra_temp.tab")
        supp_blast_runtime, supp_tab_path = run_blast(supp_params, tmp_missing_fasta, out_fname=os.path.basename(tmp_supp_blast_tab))
        log.write('Supplementary BLAST #{} finished in: {}\n'.format(rerun_count, supp_blast_runtime)); log.flush()

        if supp_tab_path and os.path.exists(supp_tab_path):
            log.write("Appending new results to {}.\n".format(os.path.basename(final_blast_tab))); log.flush()
        ...
        if os.path.exists(tmp_missing_fasta): os.remove(tmp_missing_fasta)
        if os.path.exists(tmp_supp_blast_tab): os.remove(tmp_supp_blast_tab)
    else:
        final_missing = get_missing_queries(final_blast_tab, all_fasta_qids)
        if final_missing:
            log.write("\nWARN: After {} retries, {} queries still missing.\n".format(MAX_COMPLEMENTARY_RUNS, len(final_missing))); log.flush()
    ...
    log.write("\nProcessing final BLAST table: {}\n".format(os.path.basename(params['tab']))); log.flush()
    ...
    analysis_summary = {
        ...
        "REJECTED: Element length not in range ({minlen}-{maxlen}bp)".format(minlen=params['minlen'], maxlen=params['maxlen']): [],
    ...
    with open(summary_fpath, 'w') as summ_fh:
        ...
        if 'org' in params: summ_fh.write("\nTaxids: {}".format(args.org or "N/A"))
        ...
        if blast_cols and blast_hits:
            for row in blast_hits:
                hit_dict = {str(col).strip(): str(row[i]).strip() if i < len(row) else "" for i, col in enumerate(blast_cols)}
        ...
        for qid in sorted(list(all_fasta_qids)):
            ...
            if not hits_for_qid:
                ...
                summ_fh.write("{}\tno hits in final table\t\t\tno\t\t\t\tno\n".format(qid))
            ...
            for sid in sids_in_query:
                ...
                if params['mincov'] <= avg_cov:
                    ...
                    if params['mincov'] <= avg_cov <= params['maxcov']:
                        ...
                        candidates.append({'subject_id': sid, 'avg_cov': avg_cov, 'num_contigs': len(contigs), 'contigs': contigs})

            if not candidates and all_hits_outside_coverage:
                log.write("[{}] All hits filtered out by coverage.\n".format(qid))
                summ_fh.write("{}\tno valid hits\t\t\tno\t\t\t\tno\n".format(qid))
                analysis_summary["REJECTED: All hits filtered out by coverage."].append("{}: N/A".format(qid))
                continue
            ...
            for cand in candidates:
                curr_sid, curr_avg_cov, num_contigs, contigs = cand['subject_id'], cand['avg_cov'], cand['num_contigs'], cand['contigs']
                is_valid = False
                
                if num_contigs == 1:
                    status, coords, details = one_block(contigs, q_len, params['enddist'], params['maxcov'])
                    if status == 'TERMINAL_ELEMENT':
                        elem_start, elem_end, elem_len = coords
                        if params['minlen'] <= elem_len <= params['maxlen']: is_valid = True
                        else: rejection_reasons_for_query.append(("REJECTED: Element length not in range ({minlen}-{maxlen}bp)".format(minlen=params['minlen'], maxlen=params['maxlen']), elem_len, "{} ({}..{}, {}bp, {:.2f}%)".format(curr_sid, elem_start, elem_end, elem_len, curr_avg_cov)))
                    elif status == 'FULL_COVERAGE': rejection_reasons_for_query.append(("REJECTED: Single block covers too much of query (>{maxcov}%)".format(maxcov=params['maxcov']), curr_avg_cov, "{} (-1bp, {})".format(curr_sid, details)))
                    elif status == 'INTERNAL_BLOCK': rejection_reasons_for_query.append(("REJECTED: Single block too far from query ends (>{enddist}bp)".format(enddist=params['enddist']), 0, "{} ({}, {:.2f}%)".format(curr_sid, details, curr_avg_cov)))
                elif num_contigs > 1:
                    ...
                    if elem_len > 0:
                        ...
                        rejection_reasons_for_query.append(("REJECTED: No significant gap found between blocks", 0, "{} ({:.2f}%)".format(curr_sid, curr_avg_cov)))
                
                if is_valid:
                    log.write("[{}] Candidate found in {}: {}..{} ({} bp, {:.2f}%).\n".format(qid, curr_sid, elem_start, elem_end, elem_len, curr_avg_cov))
                    summ_fh.write("{}\t{}\t{:.2f}\t{}\tyes\t{}\t{}\t{}\tyes\n".format(qid, curr_sid, curr_avg_cov, num_contigs, elem_start, elem_end, elem_len))
                    with open(coordinates_fpath, 'a') as coord_fh: coord_fh.write("{};{};{}\n".format(qid, elem_start, elem_end))
                    write_element_files(qid, qseq_content, elem_start, elem_end, params["out"], params['color'], log)
                    analysis_summary["VALID: Element found"].append("{}: {} ({}..{}, {}bp, {:.2f}%)".format(qid, curr_sid, elem_start, elem_end, elem_len, curr_avg_cov))
            ...
            if element_found_for_qid:
                ...
                log.write("[{}] No valid element found. Primary rejection reason: {}\n".format(qid, primary_reason))
                analysis_summary[primary_reason].append("{} | {}".format(qid, details))
                summ_fh.write("{}\tno valid element identified\t\t\tno\t\t\t\tno\n".format(qid))
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not is_valid:
            sys.stderr.write("\nInvalid arguments. See help (-h for details).\n")
            sys.exit(1)
        
        log_fpath = os.path.join(params["out"], 'file.log')
        ...
        for p_name, p_val in sorted(params.items()):
            if p_name not in ['qid_original_from_fasta', 'current_blastn_parts_dir']:
                log.write("  {}: {}\n".format(p_name, p_val))
        log.flush()

        with open(params['q'], "r") as q_fh:
            qseq_content = q_fh.read()
        ...
        sys.stderr.write("\nCRITICAL ERROR: Unhandled exception in main block.\n" + traceback_str)
        if log: log.write("\nCRITICAL ERROR: Unhandled exception.\n" + traceback_str); log.flush()
        ...
        if log and hasattr(log, 'write') and not log.closed:
            ...
            log.write("\n----- Analysis Breakdown by Rejection Reason -----\n")
            reason_order = [
                "VALID: Element found",
                "REJECTED: No hits in final table.",
                "REJECTED: All hits filtered out by coverage.",
                "REJECTED: Single block covers too much of query (>{maxcov}%)".format(maxcov=params.get('maxcov', 90)),
                "REJECTED: Single block too far from query ends (>{enddist}bp)".format(enddist=params.get('enddist', 50)),
                "REJECTED: No significant gap found between blocks",
                "REJECTED: Element length not in range ({minlen}-{maxlen}bp)".format(minlen=params.get('minlen', 4000), maxlen=params.get('maxlen', 50000)),
                "REJECTED: No valid element found in any hit"
            ]
            for reason in reason_order:
                value_list = analysis_summary.get(reason, [])
                if value_list:
                    count = len(value_list)
                    if reason.startswith("VALID"):
                        log.write("- {} | {}: {}\n".format(count, reason.replace("VALID: ",""), ", ".join(value_list)))
                    else:
                        display_list = [v.split(' | ')[0].strip() for v in value_list]
                        log.write("- {} | {}: {}\n".format(count, reason.replace("REJECTED: ",""), ", ".join(display_list)))
